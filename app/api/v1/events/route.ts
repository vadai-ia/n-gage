...
const CreateEventSchema = z.object({
  ...
  match_mode: z.enum(["swipe", "mosaic"]).default("mosaic"),
  super_likes_max: z.number().int().min(0).max(20).default(1),
  ...
  try {
    ...
    const event = await prisma.event.create({
      data: {
        ...
        match_mode: d.match_mode as never,
        super_likes_max: d.super_likes_max,
